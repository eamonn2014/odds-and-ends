...
output$text.wiki2 <- renderText({  # create interactive text t1
    ...
    "Some diagnostics are presented on the next tab. The diagnostics include a check of assumptions of constant variance across the range and constant difference across the range. Below there are more tabs, the first two plots are repeated but with some small modifications (just for programming practice). There is also a tab including histogram of the differences", #),
...
output$text.wiki2a <- renderText({  # create interactive text t1
    ...
    "The next tab 'Method Comp. (no replication)' is set up is a typical method comparison experiment. No replication. On the same tab are mini tabs exploring assumptions", #),
...
output$text.wiki2b <- renderText({  # create interactive text t1
    ...
    "'Method Comp. with replication' is next, there is alot of flexibility in the design. Scatter plots and Bland Altman plots are presented. We use the superb MethComp R package. The second minitab on this tab, presents an analysis assuming replicates are exchangable. The next tab assumes replicates are linked. The next mintabs A-E, presents an alternative LMM approach. There are alos mini tabs presenting boxplots, plots if we permure the reps randomly. We analyse as if each replicate were a seperate sample. we look at the assumptions on the next mini tabs and provide a data listing on the last mini tab", #),
