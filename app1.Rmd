...
inference <- reactive({
   ...
   #-------------------------------------------------
   if (nchar(i)==1 && nchar(j)==1) {
     
              res <- contrast(f, list(IV=i),
                              
                                 list(IV= j),  conf.int=0.95, #weights=table(df$IV),
                              
                              type='average')
    
    #-------------------------------------------------
    } else if (nchar(i)==1 && nchar(j)==2) {
     
              res <- contrast(f, list(IV= substr(i, 1, 1)),
                    
                                 list(IV= substr(j, 1, 1)   , IV=  substr(j, 2, 2)  ), conf.int=0.95,  #weights=table(df$IV),
                    
                    type='average') 
              
    }  else if  (nchar(i)==1 && nchar(j)==3) {
     
              res <- contrast(f, list(IV= substr(i, 1, 1)),
                    
                                 list(IV= substr(j, 1, 1)   , IV=  substr(j, 2, 2)  ,  IV=  substr(j, 3, 3)  ), conf.int=0.95,
                              #weights=table(df$IV),
                    
                    type='average') 
      }  else if  (nchar(i)==2 && nchar(j)==2) {
     
              res <- contrast(f, 
                    
                                 list(IV= substr(i, 1, 1)   , IV=  substr(i, 2, 2)  ), 
                              
                                 list(IV= substr(j, 1, 1)   , IV=  substr(j, 2, 2)  ), conf.int=0.95, #weights=table(df$IV),
                    
                    type='average') 
     }  else if  (nchar(i)==2 && nchar(j)==1) {
      
              res <- contrast(f, 
                    
                                 list(IV= substr(i, 1, 1)   , IV=  substr(i, 2, 2)  ), conf.int=0.95,
                              
                                  list(IV= substr(j, 1, 1)), #weights=table(df$IV),
                    
                    type='average') 
    }  else if (nchar(i)==3 && nchar(j)==1) {
     
              res <- contrast(f, 
                    
                                 list(IV= substr(i, 1, 1)   , IV=  substr(i, 2, 2)  ,  IV=  substr(i, 3, 3)  ), 
                                 list(IV= substr(j, 1, 1)),   conf.int=0.95,  #weights=table(df$IV),
   ...
   #-------------------------------------------------
    ...
    #   print(contrast(f, list(IV=lt[-i]),
    #                     list(IV=lt[ i]), type='average'))
    # }
    # options(datadist=NULL)
    
   return(list(res=est , L=L, H=H,i=i, j=j))   
...
# If i enter A B   [remove not A  remove not B]
# 
# if I enter AB C  [remove not AB remove not C]
# 
# if I enter A ABC [remove not AB remove not ABC]
