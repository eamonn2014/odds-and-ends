...
```{r mc2}
  ...
  # sliderInput('n_2', 'n pairs of data', value=100,
  #               min = 5, max = 1000, step=5, ticks=F) 
  # 
  # sliderInput('mu0_2', 'Initial mean  \u03BC', value=0,
  #               min = -100, max = 100, step=5, ticks=F)
  # 
  # sliderInput('eff.p_2', 'Constant difference', value = c(0),
  #               min = -25, max = 25 ,step=1, ticks=F)
  # 
  # sliderInput('sd._2', 'Common constant \u03C3', 1,
  #               min = 1, max = 100, step=1,ticks=F)
  # 
  # sliderInput('r_2', 'Correlation \u03C1', value = c(.8),
  #               min = -.9, max = .9, step=0.05, ticks=F)
  # 
  # selectInput("SD.type_2", "Select \u03C3 type",          choices = c("cons", "lin"), selected="cons")
  # 
  # selectInput("Dif.type_2", "Select difference type", choices = c("cons", "lin"), selected="cons")
  # 
  # selectInput("Pl.type_2", "Select plot",             choices = c("BA", "conv"), selected="BA")
  # 
  
  
#   methods
# a1 a2 [intercept no of methods]
# slope1 slope2 [equal no of methods]
# reps
# repsmin
# missing data 
#    sigma.mi=c(0.062,0.062), [equal no of methods]
#                 sigma.ir=c(0.02,.03), [equal no of methods]
#                 sigma.mir=c( s1 , s2) , [equal no of methods]
#                 mu.range=c( -0.5 ,1)[equal no of methods]
#   
...
# b1<-1
# b2<-1 
# s1<-0.128  #residual sd method1
# s2<-0.143  #residual sd method2

#set.seed(0)
 
# foo <- Meth.sim(Ni=samples , 
#                 Nm=sites,
#                 Nr=replicates, 
#                 nr=replicates,
#                 alpha=c(0 ,.04), 
...
textInput('alphaz', 
          strong(div(h5(tags$span(style="color:blue", 
        "A vector of method-specific intercepts for the linear equation relating the 'true' underlying item mean measurement to the mean measurement on each method.")))), "0,0.4")

textInput('betaz', 
          strong(div(h5(tags$span(style="color:blue", 
          "A vector of method-specific slopes for the linear equation relating the 'true' underlying item mean measurement to the mean measurement on each method.")))), "1,1")

textInput('repz', 
          strong(div(h5(tags$span(style="color:blue", 
           "Replicates and reps min")))), "2,1")

textInput('sigma.miz', 
          strong(div(h5(tags$span(style="color:blue", 
            "Method-specific standard deviations for a method by item random effect. Some or all components can be zero.")))), "0.062,0.062")

textInput('sigma.irz', 
          strong(div(h5(tags$span(style="color:blue", 
            "Method-specific standard deviations for the item by replicate random effect.")))), ".02,.03")

textInput('sigma.mirz', 
          strong(div(h5(tags$span(style="color:blue", 
            "A vector of method-specific residual standard deviations for a method by item by replicate random effect (residual variation). All components must be greater than zero.")))), "0.128,0.143")
 
textInput('mu.rangez', 
          strong(div(h5(tags$span(style="color:blue", 
            "The range across items of the 'true' mean measurement. Item means are uniformly spaced across the range. ")))), "-0.5 ,1")
 
textInput('mizz', 
          strong(div(h5(tags$span(style="color:blue", 
           "Fraction of the observations from each method to keep, fraction of the observations from each item to keep")))), "1 ,1")
   ...
   #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   # orange button
   actionButton("resample_2",   "Hit to simulate a new sample")
...
renderPlot({
    ...
    #          xaxs="i", yaxs="i",
    #          las=1, eqn=TRUE,  
    #          pl.type=input$Pl.type_2, 
    #          sd.type=input$SD.type_2, 
    #          dif.type = input$Dif.type_2, # cons is simulatied
    #          grid=1:9*10, digits=3,font.eqn=1  , eqax=T)
...
```{r mc2 D}
 ...
 random.sample_2 <- reactive({
   ...
   foo <- Meth.sim(Ni=nz , 
    ...
    return(list(foo=foo, miz=miz, irz=irz, mirz=mirz))
     })
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   h5(htmlOutput("text2az"))   
    ...
    # --------------------------------------------------------------------------
    h5(htmlOutput("text4z"))   
    # --------------------------------------------------------------------------
    h5(htmlOutput("text4az"))   
    # --------------------------------------------------------------------------
```
