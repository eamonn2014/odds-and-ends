...
```{r global, include=FALSE}
  ...
  # https://stackoverflow.com/questions/40172024/zoom-out-shiny-app-at-default-in-browser
 ...
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 # FUNCTIONS ROUNDING/GEN COR DATA.ROUNDING/LOGNORMAL
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
**Design your method comparison, a simple setting, no replication, samples are chosen uniformally to span the range**
...
```{r tab1q}
 ...
 # actionButton("resampleq", "Hit to simulate a new sample", icon("paper-plane"), 
  # style="color: orange; background-color:#fff ; border-color: #2e6da4; width:100%;")

  actionButton("resampleq", "Hit to simulate a new sample",
...
```{r xx1, eval=TRUE}
  ...
  y <-    before - after    # notice 1-2
  x <-   (before + after) /2
  
  w <- data.frame(x=x,y=y)
  ...
  return(list( con.diff=con.diff, con.sd=con.sd , w=w , f=f))
...
w <- r2q()$w
  ...
  a2 <- input$alphas[1]  # sd
  a1 <- input$alphas[2]
    
  eff.p <-a2-a1
  argz1 <- c(   mean(df$y),   eff.p)
  argz2 <- c(   sd(df$y),     sqrt(r1q()$true) )
  colz <-  c("black", "red")
  
  xv <- argz1[2]+sd(df$y)*1.5  # x co-ordinate of text
  
  top <- 1/sqrt(2 * pi * var(df$y) )  # height of normal density for text
 ...
 p <- ggplot(df, aes(y)) +      
  ...
  annotate("text",  x=xv, y=top*1.3, label= paste0("Estimated Mean differences = ", formatz3(mean(df$y))," and SD of differences = ",          formatz3(sd(df$y)), " (black)"))  +
  annotate("text",  x=xv, y=top*1.2, label= paste0("True Mean difference = ",      formatz3( eff.p),", and true SD of differences = " ,  formatz3(sqrt(r1q()$true)) ,  " (red)"))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  output$textmcr1 <- renderText({  # create interactive text t1
    ...
    tags$span(style="color:blue;font-weight:bold;font-size: 20px;", 
      ...
      "The agreement between methods is considered acceptable if the variability between the observations made with different methods on the same subject is not much larger than the variability between observations with the same method on this subject. How much is not much?\n"))
...
```{r not metcomp5, eval=TRUE}
   
     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
      h5(htmlOutput("textzz5")) 
      h5(htmlOutput("textzz7")) 
      h5(htmlOutput("textzz6")) 
     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
     output$textzz5<- renderText({  # create interactive text t1

    print(paste0(   
    
           tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               
   "\nTest the hypothesis that both methods have equal between-subject variances, p = ")
     ...
     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  output$textzz6<- renderText({  # create interactive text t1

    print(paste0(   
    
            tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               
   "\nTest the hypothesis that both methods have equal within-subject variances, p = ")
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     
    output$textzz7<- renderText({  # create interactive text t1

    print(paste0(   

      tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               
   "\nTest the hypothesis of equality of overall variances, p = ")
  ...
  Roy<- reactive({
   ...
   foo <- r1_2()$foo  # data
    ...
    # Standard deviation of the differences can be computed accordingly ..var of diff equals sum of var -2 x covariance
...
#  dfr1 <- dfr <- data.frame(dfr, y = y)
 ...
 # if (Nr >1 & sigma.ir ==0 ) M <- BA.est(d1, linked=FALSE)
  ...
  dfr$mu <- formatz3(mu)
  dfr$MI <- formatz3(e.mi)
  dfr$IR <- formatz3(e.ir)
  dfr$MIR <- formatz3(e.mir)
  dfr$y <- formatz3(y)
...
### Explanation {data-commentary-width="400"}
...
output$e99b <- renderText({  # create interactive text t1
    ...
    print(paste0(   
     ...
     "IR is the item×replicate component, this is a shift based on realisations from a normally distributed component ~N(0,sigma.ir). This is only for a linked experiment, for example the same items replicates receive the same shift regardless of method. So this component has nothing to do with the methods. Only one component can be estimated. Think of mixed model repeated measures (MMRM) scenario and a single treatment effect at each visit, if treated as a random effect there is only one sd to estimate.")) )
...
output$e100 <- renderText({  # create interactive text t1
    ...
    print(paste0(   
     ...
     "MI is the method×item component, you can see it affects the same method items which receive a shift, a realisation from a method specific sd, namely ~N(0,sigma.mi). As explained by Bendix Carstensen 'The introduction of a third method allows the separation of the
method×item variation between the methods. In the case of only two
methods the deviations of the empirical method means for a given item
can only be taken to be of opposite sign from the method×item means
μi + αm (this is how μi is determined). But with three methods, we
have three different distances to the method×item means.")) )
...
output$e101 <- renderText({  # create interactive text t1
    ...
    print(paste0(   
     ...
     "MIR is the method×item×rep component, repeatability or residual you can see it affects each replicate which receive a realisation from a method specific ~N(0,sigma.mir) distribution")) )
...
# Paired data {data-icon="fa-pencil"}

## Column {.sidebar}

**Design, no replication**
...
renderPlot({
   ...
   L <- roundUp(L)
    ...
    BA.plot( m, model=NULL, repl.conn=TRUE, col.lines="blue",
             ...
             sd.type=input$SD.type, 
             dif.type = input$Dif.type, # cons is simulated
...
output$text4a <- renderText({  # create interactive text t1
 ...
 output$text5 <- renderText({  # create interactive text t1
    ...
    tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               ...
               "After anti-logging, the contrast ") 
...
# Diagnostics {data-icon="fa-pencil"}
...
renderPlot({
 ...
 pitman <- cor.test(w$x, w$y)$p.value
  ...
  ggplot(w, aes(x=x, y=y))+
    ...
    stat_regline_equation(label.x = min(w$x), label.y = max(w$r))+
    theme_bw() +
    labs(title = paste0("Regression of differences on averages. Testing hypothesis slope=0, p=", formatz3(con.diff), ". Note: the constant SD assumption \ncan also be evaluated using the \np-value from the correlation of differences on the average (known as Pitman's test) p=", formatz3(pitman)),
         x = "Averages",
         y = "After - before")
...
w <- r2()$w
  ...
  top <- 1/sqrt(2 * pi * var(df$y) )  # y co ordinate of text
 ...
 p <- ggplot(df, aes(y)) +      
  ...
  annotate("text",  x=xv, y=top*1.3, label= paste0("Estimated Mean differences = ", formatz3(mean(df$y))," and SD of differences = ",          formatz3(sd(df$y)), " (black)"))  +
  annotate("text",  x=xv, y=top*1.2,   label= paste0("True Mean difference = ",      formatz3( input$eff.p),", and true SD of differences = " ,  formatz3(sqrt(r1()$true)) ,  " (red)"))
...
```{r placeholder3, eval=TRUE}

```

# T-test {data-icon="fa-cog"}

## Column {.sidebar}

Here we define the parameters of a log normal distribution and a proportional change (independent groups)
...
```{r}
  ...
  qc <- ifelse(((qclevel<=lo) && (hi<=1/qclevel)), paste0("PASS specification of ", qclevel), paste0("FAIL specification of ", qclevel))
...
# Anova contrasts {data-icon="fa-pencil"}
...
output$text101 <- renderText({  # create interactive text t1
   ...
   #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    tags$span(style="color:black;font-weight:bold;font-size: 20px;",
               ...
               "Matrix multiplication [weights] [VARCOV] [weights'] and take the square root to get se")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          output$text107a <- renderText({  # create interactive text t1

    print(paste0(   
    
    tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               
               "Contrasts of levels with intercept (anti-logged)") 
...
```{r, eval=TRUE}
 ...
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  renderPrint({
    ...
    model1 <- inference()$lower
     model2 <- inference()$upper
    print(model1)
    print(model2)
...
inference <- reactive({
   ...
   conf.int = 0.95
    ...
    # left side
    da <- do.call("gendata", list(fit, factors = a, expand = TRUE)) # creates a df of levels of interest
    ...
    # if you don't do this design matrices could be wrong
      mm <- max(mall)
    if (mm > 1 && any(mall == 1)) {
        if (ma == 1) 
...
# Wiki {data-icon="fa-book"}

```{r}
  h5(htmlOutput("text.wiki2a"))
  
  h5(htmlOutput("text.wiki2b"))
  
  h5(htmlOutput("text.wiki2c"))
...
# --------------------------------------------------------------------------
tags$style(type = "text/css", "#map {height: calc(1000vh - 80px) !important;}") # height of page so no clipping text
...
output$text.wiki <- renderText({  # create interactive text t1
    ...
    "A simple experiment in which any difference is constant across the range and the standard deviation is also constant and common to both methods is presented on the 'Paired data' tab. Here two groups are set up using a between sample variation and a defined correlation. We determine the mean and intervention effect. If for example two measurement methods agree we expect there to be positive correlation. That is, if one method measures a sample to have a high value we would expect the second method to have a similar high value. We present a plot of the data using boxplots and join the same sample with a thin line, beside it a simple scattterplot. Top right the default plot is the familiar Bland Altman plot with 95% limits of agreement and conversion equation. There is an option to show instead the inference on a scatter plot. There are user options too for the analysis, bottom left. Here, the difference can be analysed depending on the averages assuming it is constant (default) or linear. The standard deviation can be analysed depending on the averages assuming it is constant (default) or linear", #),
...
output$text.wiki2 <- renderText({  # create interactive text t1
    ...
    "Diagnostics are presented on the next tab. The diagnostics include a check of assumptions of constant variance across the range and constant difference across the range. Below there are more tabs, the first two plots are repeated but with some small modifications (just for programming practice). There is also a tab including histogram of the differences", #),
...
output$text.wiki2a <- renderText({  # create interactive text t1
    ...
    "The first tab 'No reps' is a typical method comparison experiment, that is no replication, an inferior design. On the same main tab are mini tabs exploring assumptions", #),
...
output$text.wiki2b <- renderText({  # create interactive text t1
    ...
    "'reps!' is next, there is alot of flexibility in the design. Scatter plots and Bland Altman plots are presented. We use the superb MethComp R package. The second mini tab on this tab, presents an analysis assuming replicates are exchangable. The next tab assumes replicates are linked. The next mini tabs A-E, presents an alternative LMM approach. There are also mini tabs presenting boxplots, plots if we permute the reps randomly. We then analyse as if each replicate were a seperate sample. We examine assumptions on the next mini tabs and provide a data listing on the last mini tab", #),
...
output$text.wiki2c <- renderText({  # create interactive text t1
    ...
    "The next tab 'Method Comp. listing' here we show the ideas as plain as possible behind the simulation and theory", #),
...
output$text.wiki4 <- renderText({  # create interactive text t1
    ...
    " The Anova (linear model) contrasts tab, again not directly related to method comparison, allows the user to define 4 lognormal distributed variables of user defined sample sizes, sds and proportional differences.  The idea behind this tab is to perform and understand contrasts. On the left is Frank Harrell's contrast function and on the right we step through how a contrast is calculated
    ", #),
...
<!-- ttest diagnostics\ -->
<!-- bayesian\ -->
<!-- photo to aid understanding -->

```{r wiki}
    
```

# References {data-icon="fa-pencil"}
...
```{r, refs}
    ...
    div(p(" "))
       ...
       tags$a(href = "https://fontawesome.com/v4/examples/", target="_blank",
           tags$span(style="color:blue", "[2] icons"),) 
    div(p(" "))
     
     tags$a(href = "https://cran.r-project.org/web/packages/MethComp/index.html", target="_blank",
           tags$span(style="color:blue", "[3] MethComp R package"),) 
    div(p(" "))
    
     tags$a(href = "https://duckduckgo.com/?q=Bendix+Carstensen+method+comparison&atb=v314-1&ia=web", target="_blank",
           tags$span(style="color:blue", "[4] Author of MethComp R package"),) 
    div(p(" "))
