...
#n <- input$n
 ...
 library(ggplot2)
  ...
  ggplot(w, aes(x=x, y=r))+
    ...
    stat_regline_equation(label.x = min(w$x), label.y = max(w$r))+
    theme_bw() +
    labs(title = paste0("1 regressing the differences on the averages and then regressing the absolute residuals from this on the averages, p=", con.sd),
         x = "Averages",
         y = "Absolute residuals")
...
#n <- input$n
 ...
 library(ggplot2)
  ...
  ggplot(w, aes(x=x, y=y))+
    ...
    stat_regline_equation(label.x = min(w$x), label.y = max(w$r))+
    theme_bw() +
    labs(title = paste0("Regression of differences on averages, p=", con.diff),
         x = "Averages",
         y = "After - before")
