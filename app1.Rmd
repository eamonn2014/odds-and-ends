...
```{r tab1}
  ...
  # shinyWidgets::sliderTextInput("Mu","Mu",
  #                           choices=c(0.001, 0.01, .1 ,1, 10, 100, 1000, 10000, 1e5 , 1e6),
  #                           selected=4000, grid = T)
#
...
```{r, main power tab2}
 ...
 div(h5(tags$span(style="color:blue", "Enter True Population values:")))
      ...
      textInput("v4", div(h5(tags$span(style="color:blue", "Confidence Level"))), value= ".90")
  ...
  random.sample2 <- reactive({
    ...
    #------------------------------------------------------------------------------
        return(list(
         ...
         mu=mu, sd=sd, zz=zz, logN.mu=logN.mu, logN.SD=logN.SD, delta=delta, A.GM=A.GM, B.GM=B.GM, zx=zx, A=A, B=B, N=N, conf=conf, qc=qc,  mux=mux
...
MASS::truehist(A, 
              yaxt='n' ,
              nbins=br,  axes=FALSE,
               main=paste0("N=",N," realisations from log-normal mu=",p3(logN.mu),", SD=",p3(logN.SD),", Geometric Mean=",p3(A.GM)), col = "#75AADB", 
...
MASS::truehist(B, 
                yaxt='n' ,
                nbins=br,  axes=FALSE,
               main=paste0("N=",N," realisations true log-normal mu=",mux*zx,", SD=", p3(logN.SD),", Geometric Mean=",p3(B.GM) ," Treatment effect ",mux*zx," / ",mux,"=",zx), 
...
MASS::truehist(log(A), yaxt='n' , nbins=br,  axes=FALSE,
               main=paste0("N=",N," realisations true Normal mu=",p3(mu),", SD=", p3(sd),""), col = "#75AADB", border = "white",  xlab="log scale")
...
MASS::truehist(log(B), yaxt='n' , nbins=br, axes=FALSE, 
               main=paste0("N=",N," realisations true Normal mu=",p3(mu+delta),", SD=",p3(sd)," Treatment effect ", p3(mu+delta)," - ",p3(mu),"=",
...
```{r}
  ...
  sliderInput('nx', 'n pairs of data', value=100,
                min = 5, max = 1000, step=5, ticks=F)

  sliderInput('mu0x', 'Initial mean  \u03BC', value=0,
                min = -100, max = 100, step=5, ticks=F)
  
 
  sliderInput('eff.px', 'Effect of treatment', value = c(0),
                min = -25, max = 25 ,step=1, ticks=F)
  
   sliderInput('rx', 'Correlation', value = c(.8),
                min = -.9, max = .9, step=0.05, ticks=F)
     
  sliderInput('sd.x', 'Common SD', 1,
                min = 1, max = 100, step=1,ticks=F)
  
  selectInput("SD.typex", "Select SD type", choices = c("cons", "lin"), selected="cons")
  
  selectInput("Dif.typex", "Select difference type", choices = c("cons", "lin"), selected="cons")
  
  selectInput("Pl.typex", "Select difference type", choices = c("BA", "conv"), selected="BA")
