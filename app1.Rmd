...
### Bland Altman or conversion plot, comparison and plot can be selected by user (lines joining dots only applicable for linked replicates). Note: I am not sure why equation does not match mixed model exactly!?
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  output$textmcr1 <- renderText({  # create interactive text t1
    ...
    tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               ...
               "Analyse a simulated study that includes techical replication. Replicates are needed to assess repeatability. The assumptions (contant \u03C3 and constant difference are evaluated using the data as if there were no replication. boxplots are presented, but really are not very informative. The analysis itself explicitly takes into account the study design and replication. We analyse the data in two ways considering the replicates are linked and considering they are exchangable. We also perform a hypothesis test to test whether the repeatabilty methods are different to each other. We also permute the replicates and plot. The limits should be similar if exchangeable.\n"),
    
        tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               
               "Changing differences can be a manifestation of different repeatabilities. So replicates allow this conclusion to be drawn"))
...
```{r mcomp2.4}
 ...
 renderPlot({
    ...
    AA <- ggplot(data=d, aes(y=y) ) +
      ...
      xlab("Method [dots indicate reps, items by coloured lines]") + ylab("y") +  
