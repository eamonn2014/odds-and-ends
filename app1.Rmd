...
ggplot(df, aes(y)) +        # Draw histogram with density
  geom_histogram(aes(y = ..density..),
                 colour = "white", fill = "cornflowerblue", size = 0.1) +
  stat_function(fun = dnorm,
                args = list(mean = mean(df$y),
                            sd = sd(df$y)),
                col = "black",
                size = 1.5) +
      theme_bw() +
  
  annotate("text", x=min(df$y), y=.8, label= paste0("Mean = ", formatz3(mean(df$y))," SD = ", formatz3(sd(df$y))))  
