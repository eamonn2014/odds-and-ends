...
The inputs 'probability of success' and 'number of successes' dictate the PMF bottom left, the red line is the PMF whilst the bar are from a simlation. The input parameters this time are then converted to mu and alpha and displayed bottom right.\n\n"
...
tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
   ...
   "The second panel allows the user to enter the 'Probability of success (p)' and the 'Number of successes (r)'. The top panel presents the PMF for the probability for number of attempts need to reach r successes. Whilst the lower panel presents the number of failed trials before the rth success, using the equivalent mu and alpha. Underneath are the  probability mass functions."
)   ,
...
tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
   ...
   "The fourth tab explores the Poisson-Gamma mixture distribution allowing the user to enter r and β of a gamma distribution which if the Poisson mean. the resulting distribution is presented top left. The equivalent Negative binomial is presented in the center panel and the right panel converts the middle panel's size and probabilty to mu and alpha. Below is an illustration of sampling from a Poisson Gamma mixture and compares the sample with the negative binomial distribution\n\n"
)   ,
