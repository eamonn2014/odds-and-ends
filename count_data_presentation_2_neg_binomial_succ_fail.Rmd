...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# number of successes function, to simulate neg binomial 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
 foo <- function(p = 0.5, s = 4) { # p prob of success; s 
  
  i <- 0 # count of how many attempts we need to get s successes
...
#-------------------------------------------------------------------------------------------------------
 ...
 output$intro1 <- renderText({  # create interactive text t1
    ...
    print(paste0(
     ...
     tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
               ...
               "Exploration of a not so well known, but common distribution used in the analysis of count data...the negative binomial distribution also know as the gamma-Poisson distribution") ,
 ...
 output$intro3 <- renderText({  # create interactive text t1
   ...
   output$intro5 <- renderText({  # create interactive text t1
   ...
   "In the first tab the user can input the mean mu, and alpha of the negative binomial distribution and the resultant probability mass function (PMF) is displayed, top left. On the right, mu and alpha are converted to the probability of success and number of successful attempts sought.\n\n

The inputs 'probability of success' and 'number of successes' dictate the PMF bottom left, the red line is the PMF whilst the bars are from a simulation. The input parameters this time are then converted to mu and alpha and displayed bottom right.\n\n"
...
tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
   ...
   "The second panel allows the user to enter the 'Probability of success (p)' and the 'Number of successes (r)'. The top panel presents the PMF for the probability for number of attempts need to reach r successes. The lower panel presents the number of failed trials before the rth success, using the equivalent mu and alpha. Underneath are the  probability mass functions."
)   ,
...
tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
   ...
   "The QC tab allows the user to enter the 'Probability of success (p)' and the 'Number of successes (r)' and the associated PMF presented for the attempts for r successes. Below are a number of calculations to check understanding, reproducing means and variances."
)   ,
...
tags$span(style="color:black;font-weight:bold;font-size: 20px;", 
   ...
   "The fourth tab explores the Gamma-Poisson mixture distribution allowing the user to enter r and β of a gamma distribution which is the Poisson mean. The resulting distribution is presented top left. The equivalent Negative binomial PMF is presented in the center panel and the right panel converts the middle panel's size and probabilty to mu and alpha. Below that is an illustration of sampling from a Gamma-Poisson mixture and compares the sample with the negative binomial distribution\n\n"
)   ,
...
Negative binomial PMFs {data-icon="fa-cog"}
===

Column {.sidebar}
-----------------------------------------------------------------------

**The Probability Mass Function is explored**
...
```{r, tab1 plot2, eval=TRUE}
 ...
 # if (input$alpha_a==0) {a=1} else { a = input$alpha_a } # stop plot error
  ...
  # alphaz = zz[2][[1]]
  # simulation bar plot
  #!!!!!!!!!!!!!!!!!!!!!!this above is actually not used
  ...
  x <- table(factor(number_of_attempts, levels = s: max(number_of_attempts))) # pad out so levels of no counts included
...
Negative binomial PMFs (ii) {data-icon="fa-rocket"}
===
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  # sanity check on mean and variance and report them as qc check in plot
...
Gamma Poisson {data-icon="fa-rocket"}
====================================
...
```{r input4}
  ...
  sliderInput('nn', 'Gamma-Poisson mixture sims', value=10000, #.5
...
Gamma-Poisson mixture example


Column 
-----------------------------------

### Generate Gamma-Poisson mixture   


```{r}

#generate a Gamma-Poisson mixture
...
renderPlot({
  ...
  b <- barplot(d, 
    ...
    if(input$percx2) {
        ...
        x  <-0:length(b) # % onto top of bars
        y1 <- d + 0.02 # gap above bar
        z  <- round(d,3) *100
        z  <- paste0(z,"%")
        text(x=b[x+1],y=(y1),labels=z, cex = .8)
  }
par(mar=c(5.1, 4.1, 4.1, 2.1), mgp=c(3, 1, 0), las=0)
 })
...
### Use the mu and alpha in the middle panel and plot this
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  b <- barplot(yb, xlim=c(0,xmax+2), ylab = "Probability", xlab="Count of failed attempts (X-r)", col='lightgreen',
          ylim=c(0,ymax),
          main =  paste0("PMF of Negative Binomial\n \u03BC = ",formatz2(mu)," \u03B1 = ",formatz2(alpha_a),", variance = ", formatz3(mu+alpha_a*mu^2),""), # mu+alpha_a*mu^2* alpha_a
          names.arg= as.character(xx))
...
- [My presentation on count data](https://github.com/eamonn2014/count-data/blob/main/Count%20data%20analysis%20using%20R%2012Aug2022.pptx)
