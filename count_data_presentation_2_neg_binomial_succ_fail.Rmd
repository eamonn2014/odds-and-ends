...
```{r side3}
  ...
  sliderInput('Px1', 'Probability of Success (p)', value=.5,
                min = 0.01, max = 1, step=.01, ticks=F)
  
  sliderInput('sx1', 'No of successess (r)', value = c(3),
                min = 1, max = 50 ,step=1, ticks=F)
...
```{r}
 ...
 # Column {data-width=400 data-height=1200}
 #
...
```{r qc1, include=TRUE, echo=TRUE, eval=TRUE}
  ...
  xx <- reactive({
        ...
        v2e  <- var(my_dat1)         ## 
        #v2s  <- r * (1 - p) / p^2 # same formula as V2s
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
```{r qc2, include=TRUE, echo=TRUE, eval=TRUE}
    ...
    qc2 <- reactive({
         ...
         value <-   0:1000000        # these lines taken from when my ttests won't do code 
         freq  <-  e_dnbinom(value, alpha=alpha_a,mu=mu)*100
         mux   <-  sum(freq*value)/sum(freq)
         vax   <- (sum(freq*(value-mu)^2)/(sum(freq)-0) ) # I changed to 0 from 1 !
       
         return(list( mu=mux, v=vax , #sim
                      
                    mu1=mu, a=alpha_a  )) # from function
...
```{r qc2a }
    ...
    renderPrint({
        ...
        f3 <-  qc2()$mu1
        cat("mu from user function\n") 
        print(f3)

        f4 <-  qc2()$a
        cat("alpha from user function\n") 
        print(f4)
...
```{r qc3}
  ...
  HTML("  With regard to negative binomial regression the variance can be written as ")
  ...
  HTML("  These two formulations can introduce confusion, so it is important to clarify if a dispersion value is k or alpha. R code below showing syntax.")
 ...
 withMathJax(
  ...
  br()
   ...
   HTML("Viewing µ and sigma as primary, we ignore the combinatorial motivation for
