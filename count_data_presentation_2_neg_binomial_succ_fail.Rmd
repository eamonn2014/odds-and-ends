...
runtime: shiny
---
Probability mass functions {data-icon="fa-home"}
===
...
Probability mass functions 2 {data-icon="fa-rocket"}
===
...
```{r input4}
  ...
  sliderInput('nn', 'Number of random variates', value=10000, #.5
                min = 100, max = 100000, step=100, ticks=F)

  sliderInput('rx', 'xxxxxxxxxxxxxxxxx (r)', value=.4, #.5
                min = 0.01, max = 100, step=.01, ticks=F)
  
  sliderInput('betax', 'Beta', value = c(2), #3
                min = .1, max = 40 , step=.1, ticks=F)

  sliderInput('x_rangex2x', 'Plot x-range', value = c(0,10),
          min = 0, max = 80,step=5,ticks=F)
  
  sliderInput('y_rangex2x', 'Plot y-range', value = c(0,1),
...
ff <- reactive({
   ...
   negbin <- round(dnbinom(0:max(xx), r, beta/(1+beta)), 3)
...
renderPlot({
  ...
  b <- barplot(d, 
               xlim=c(0,xmax+2),  
               ylim=c(0,ymax),
               main = paste0("The empirical distribution of the Poisson-Gamma mixture\nThe Poisson mean is rgamma(shape= ", r,", rate= ", beta,")\n"),
               col="orchid1", 
               ylab="Probability", 
               xlab=paste("Counts"),
               names.arg= as.character(xx))
...
renderPlot({
 ...
 # d <- ff()$d
  ...
  b <- barplot(d, 
               xlim=c(0,xmax+2),  
               ylim=c(0,ymax),
               main = paste0("PMF of Negative Binomial( size= ",r," , prob= ",beta," )\n"),
               col="orchid1", 
               ylab="Probability", 
               xlab=paste("Counts"),
               names.arg= as.character(xx))
...
### xxxxxxxxxxx {data-height=1200}


```{r qc2.1, include=TRUE, echo=TRUE, eval=TRUE}
...
```{r qc2.2, include=TRUE, echo=FALSE, eval=TRUE}
 ...
 renderPrint({
       ...
       cat("The empirical distribution (first line below) of the mixture agrees very closely
with the pmf of the Negitiva binomial (second line), se of neg bin bottom line\n") 
       print(f)
    })
