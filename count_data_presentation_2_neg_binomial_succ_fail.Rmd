...
renderPlot({
  ...
  b <- barplot(d, 
               xlim=c(0,xmax+2),  
               ylim=c(0,ymax),
               main = paste0("\nProbability of ",s," successes on kth trial when probability of success is ",formatz2(P),
                                    ". \nAverage number of trials \u03BC (r/p) = ",formatz2(mu) ,";
                              \nThe expected number of failures prior to r successes is r(1-p)/p = ",formatz2(ef)," \nwith variance r(1-p)/p^2 = ",formatz2(v),".\nAverage number of successes rp/(1 − p) = ",formatz2(mu2),". \nCounting the number of successes given the number r of failures, \nthe variance is rp/(1 − p)^2 = ",formatz2(v2),"\n"),
...
xx <- reactive({
      ...
      my_dat <- rnbinom(n = 10000, size = r, prob = p) + r
      mu1 <- mean(my_dat) # end of line 1
      mu2 <- r / p
      
      my_dat <- rnbinom(n = 10000, size = r, prob = p)
      mu3 <- mean(my_dat) # start of line 2
      mu4 <- r*(1-p)/p
      ...
      A <- rnbinom(n = 10000, size = r, prob = 1-p)
      mu5 <- mean(A)   # start of line 3
      v3 <- var(A)    # end of line 3
 ...
 return(list( mu1=mu1, mu2=mu2, mu3=mu3, mu4=mu4, mu5=mu5, v1=v1,v2=v2, v3=v3))
